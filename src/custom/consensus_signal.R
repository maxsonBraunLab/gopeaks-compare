...
if (!dir.exists("data/figures/sample_signal")) {
  ...
  unlink("data/figures/sample_signal", recursive = TRUE)
  dir.create("data/figures/sample_signal")
}
...
for (i in 1:nrow(peak_groups)) {
  ...
  out_pdf <- paste0("data/figures/consensus_signal/", temp_condition, "_", temp_mark, ".pdf")
  out_png <- paste0("data/figures/consensus_signal/", temp_condition, "_", temp_mark, ".png")
...
for (i in 1:nrow(peak_groups)) {
  ...
  temp_condition <- as.character(peak_groups[i, "condition"])
  temp_mark <- as.character(peak_groups[i, "mark"])
  print(paste("Exporting sample peak size for", temp_condition, temp_mark))

  temp_df <- peak_size %>% 
    filter(condition == temp_condition) %>%
    filter(mark == temp_mark)
  
  out_pdf <- paste0("data/figures/sample_signal/", temp_condition, "_", temp_mark, ".pdf")
  out_png <- paste0("data/figures/sample_signal/", temp_condition, "_", temp_mark, ".png")
  plot_title <- paste(temp_condition, temp_mark, "Peak Size")
