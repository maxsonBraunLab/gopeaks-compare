...
# if (!dir.exists("data/figures/consensus_peak_plots")) {
# 	dir.create("data/figures/consensus_peak_plots")
# } else {
# 	unlink("data/figures/consensus_peak_plots", recursive = TRUE)
# 	dir.create("data/figures/consensus_peak_plots")
# }

if (!dir.exists("data/figures/peak_plots")) {
  dir.create("data/figures/peak_plots")
} else {
  unlink("data/figures/peak_plots", recursive = TRUE)
  dir.create("data/figures/peak_plots")
}
...
# peak_size_list <- lapply(list.files("data/consensus", pattern = "*.bed", full.names = TRUE), function(x) {
...
#   out_pdf <- paste0("data/figures/consensus_peak_plots/", temp_condition, "_", temp_mark, ".pdf")
#   out_png <- paste0("data/figures/consensus_peak_plots/", temp_condition, "_", temp_mark, ".png")
...
# ggsave("data/figures/consensus_peak_plots/consensus_annotation.pdf", p, width = 16, height = 9, units = "in", dpi = 600)

# Peak plots per sample ---------------------------------------------------------------------------

peak_size_list <- lapply(list.files("data/peak_counts", pattern = "*.bed", full.names = TRUE), function(x) {
...
for (i in 1:nrow(peak_groups)) {
  ...
  temp_condition <- as.character(peak_groups[i, "condition"])
  temp_mark <- as.character(peak_groups[i, "mark"])
  print(paste("Exporting peak plots for", temp_condition, temp_mark))

  temp_df <- peak_size %>% 
    filter(condition == temp_condition) %>%
    filter(mark == temp_mark)
  
  out_pdf <- paste0("data/figures/peak_plots/", temp_condition, "_", temp_mark, ".pdf")
  out_png <- paste0("data/figures/peak_plots/", temp_condition, "_", temp_mark, ".png")
  plot_title <- paste(temp_condition, temp_mark, "Peak Plot")
